...
On GPU: matmul (Q8_0 and FP32), FFN residency, attn+FFN residual (WO+RMSNorm+FFN, parallel RMSNorm apply), attention (+ softmax), KV-cache. Disable residual with `GGUF_ATTN_FFN_RESIDENCY=0`. RoPE stays on CPU.
