...
On GPU: matmul (Q8_0 and FP32), FFN residency (gate/up/SwiGLU/down), attention (+ softmax), KV-cache; optional `GGUF_ATTN_FFN_RESIDENCY=1` for WO+residual+FFN fused. RMSNorm/RoPE stay on CPU by default (PCIe / single-thread RMSNorm).
