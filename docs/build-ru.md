...
На GPU: matmul (FP32, Q8_0, Q4_0, Q4_K) с CUDA Graphs (полный HtoD+kernel+DtoH или kernel-only, если vec уже на GPU), FFN residency, QKV+RoPE+attn residency, attn+FFN residual (WO+RMSNorm+FFN, parallel RMSNorm apply), attention (+ softmax), KV-cache. Отключить: `GGUF_QKV_RESIDENCY=0` / `GGUF_ATTN_FFN_RESIDENCY=0`. Освобождать VRAM через `Engine.Close()`.
